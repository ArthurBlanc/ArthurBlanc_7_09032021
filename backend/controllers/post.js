...
exports.getAllPosts = (req, res, next) => {
	sql.query(
		"SELECT users.nom, users.prenom, posts.id, posts.userId, posts.content, posts.likes, posts.dislikes, posts.date AS date, posts.image AS postImage, posts.categoryId, thumbs.liked, thumbs.disliked FROM users INNER JOIN posts ON users.id = posts.userId LEFT JOIN thumbs ON thumbs.postId = posts.id ORDER BY date DESC",
...
exports.getAllCategoryPosts = (req, res, next) => {
	...
	sql.query(
		"SELECT users.nom, users.prenom, posts.id, posts.userId, posts.content, posts.likes, posts.dislikes, posts.date AS date, posts.image AS postImage, posts.categoryId, thumbs.liked, thumbs.disliked FROM users INNER JOIN posts ON users.id = posts.userId LEFT JOIN thumbs ON thumbs.postId = posts.id WHERE posts.categoryId = ? ORDER BY date DESC",
		[categoryId],
		(error, results, fields) => {
			if (error) throw error;
			return res.status(200).json(results);
...
exports.getOnePost = (req, res, next) => {
	...
	sql.query(
		"SELECT users.nom, users.prenom, posts.id, posts.userId, posts.content, posts.likes, posts.dislikes, posts.date AS date, posts.image AS postImage, posts.categoryId, thumbs.liked, thumbs.disliked FROM users INNER JOIN posts ON users.id = posts.userId LEFT JOIN thumbs ON thumbs.postId = posts.id WHERE posts.id = ?",
		[postId],
		(error, results, fields) => {
			if (error) throw error;
			return res.status(200).json(results);
